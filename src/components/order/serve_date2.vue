...
<script>
      import {changeDate,dateList} from "@/components/order/js/date_js2"
    ...
    export default {
        data() {
            return {
              ...
              timeList:[
                ...
                {name:"周一",time:"",isw:true},
                {name:"周二",time:"",isw:true}
              ],
              selectorTime:'',
              date:[],
              current:"",
        ...
        methods: {
          selector(index){
           ...
           if(index==0){
//             console.log(this.timeList2[index]);
...
//              }
           ...
           if(index===0&&this.state&&this.hous){
...
//          let date = new Date();
//          let day=24,second=60*60*1000;
//          console.log( this.$moment((date-0)+day*3*second).format('YYYY-MM-DD HH:mm:ss'));
...
//            if(fullName[y].labelId.substring(0,6).indexOf("003001")===-1&&false){
//                  this.state=false;
...
//                  break;
//              }
            let targetDate=Date.parse("2018/02/12 20:00:00"),currentDate =  new Date();
            this.hous=targetDate>Date.parse(currentDate);
            if(fullName[y].farterId==="003"&&targetDate>Date.parse(currentDate)){
                this.timeList.forEach((v,i)=>{
                  v.time=`02/2${i+1}`;
                  this.date[i]=`2018-02-2${i+1}`
                });
//              this.timeList=[
//                {name:"周五",time:"",isw:true},
//                {name:"周五",time:"",isw:true},
...
//                {name:"周五",time:"",isw:true}
//              ]
                this.timeList[0].name="今天";
                this.timeList[0].time=this.$moment(currentDate).format("MM/DD");
                this.date[0]=this.$moment(currentDate).format("YYYY-MM-DD");
                this.timeList[2]["name"]="周五";
                this.timeList[3]["name"]="周六";
                this.timeList[4]["name"]="周日";
//                this.timeList[5]["name"]="周一";
//                 v.name=
//                });
              if(this.$moment(currentDate).format("DD")-0<12){
                this.timeList[1].name="明天";
                this.timeList[1].time=this.$moment(targetDate).format("MM/DD");
                this.date[1]=this.$moment(targetDate).format("YYYY-MM-DD")
              }else{
                this.timeList[1].isw=false;
                  this.timeList.forEach((v,i)=>{
                    v.time=`02/2${i+1}`;
                    this.date[i]=`2018-02-2${i+1}`
                  });
              ...
              let currentDate=Date.parse(new Date()),targetDate=Date.parse("2018/02/23 00:00:00")
              targetDate<currentDate?this.state=true:this.state=false;
            }
          }
          this.selector(index);
