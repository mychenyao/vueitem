...
<script>
  export default{
    ...
    created(){
      ...
      _this.$axios.post(_this.$allUrl.qu_yu+`${dd_skill[0].id}`).then((data)=>{
        if(data.data.code=='0000'){
          if(data.data.result[0].id){   //如果默认第一个为空就给无的提示
            _this.address_region=data.data.result
            _this.$axios.post(_this.$allUrl.qu_yu+data.data.result[0].id).then((data)=>{
              if(data.data.code=='0000'){
    ...
    methods:{
      region_click(item,index){ //点击省份
        ...
        this.$axios.post(this.$allUrl.cheng_shi+'id='+item.id).then((data)=>{ //点击哪个省就去请求哪个省下面市的数据
          _this.address_street=data.data.result;
        });
      },
