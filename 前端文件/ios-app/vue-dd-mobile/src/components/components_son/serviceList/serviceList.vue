...
//              {label: '特高空：200元', value: '特高空'},
    ...
    computed:{
      ...mapGetters([  //首页订单的数据
        "order_service_list", //获取清单的数据  //挂起数据
    ...
    created(){
      let _this=this;
      // _this.maintenance_click(item,index)
      _this.service_price_list=_this.$store.getters.order_service_list; //服务清单  //挂起数据
      _this.service_list=_this.$store.getters.order_service_price_list;//服务清单价格列表
      console.log(_this.service_price_list,"898998989999999999999999999价格。配件")
      console.log(_this.service_list,"898998989999999999999999999楼层")
...
//          let repair=['小修','中修','大修','检修'];
          let repair=[];
          arr.map((arr1,index1)=>{
            repair.push(arr1.serviceInfo.name)
            _this.options.push({label: arr1.serviceInfo.name+':'+arr1.serviceInfo.price2DiscountFee+'元', value: repair[index1]});
      ...
      maintenance_click(item,index){ //维修费确定按钮
        ...
        _this.$set(_this.accessories_show_none,index,false);
        // _this.service_price_list.services.map((item,index)=>{
        //    itemData = item
        // })
        // if(itemData.price2 > 0){
        //   console.log("我点击了")
        //   if(itemData.price2)
        //    _this.repair_value_text['repair_value_text'+ index]=itemData.serviceName;
        //    _this.service_repair_total_price_arr[index]=itemData.price2;
        //    _this.repair_value_num['repair_value_num'+ index]=itemData.price2;
        //    itemData.price2 = 0
        // }else{
            ...
            if(_this.repair_value==arr1.serviceInfo.name){
              _this.repair_num[index]=index1; //看是配件费里的数组的第几个
                ...
                _this.service_repair_total_price_arr_id[index]=arr1.serviceInfo;//维修ID信息集合
                  ...
                  _this.repair_value_text['repair_value_text'+ index]=arr1.serviceInfo.name;

                _this.repair_arr2.map((arr2,index2)=>{
                      console.log(index2,"你好啊")
...
//          _this.total_value();//所有的总数合计
      ...
      serviceMountings_click(item,index){//配件费确定按钮
        let _this=this;
        _this.$set(_this.repair_show_none,index,false);
        // if(_this.service_price_list.mountings.length > 0){
        //   console.log("我是id我相等")
        //       _this.service_price_list.mountings.map((item2,index)=>{
        //         console.log(item2,"我是id我相等")
        //           _this.service_list[_this.repair_num_confirm][_this.repair_num].serviceMountings.map((item1,index1)=>{
        //               if(item2.serviceMountingsId == item1.id){
        //                 // console.log("我是id我相等")
        //                     item1.num = item2.serviceMountingsCount
        //                     if(item1.serviceMountingsPrice != 0){
        //                        console.log("我是价格我不相等")
        //                         _this.service_total_price_arr[index].push(item1)
        //                     }
        //               }
        //           })
        //       })  
...
//          _this.high_value='1-2';
         console.log( _this.service_list[index][0].highAltitudeSettings[0].floorStart+"-"+ _this.service_list[index][0].highAltitudeSettings[0].floorEnd)
        }
      ...
      high_fees_click(item,index){ //高空费确定按钮
        ...
        //    itemData1 = item
        // })
        //  if(itemData1.price3 >0){
        //     console.log("我点击了")
        //     hign_data=itemData1.price3;
        //     _this.high_value_num['high_value_num'+ index]=itemData1.price3;
        //     _this.service_list[index][0].highAltitudeSettings.map((arr,index1)=>{
        //       console.log(arr,"djsadfksafksdkfksdfkhk")
        //       if(itemData1.price3 == arr.price){
        //            _this.high_value_text['high_value_text'+ index]=arr.floorStart+"-"+arr.floorEnd+"层";
        //       }
        ...
        // }else{
         //控制服务清单高空费楼层
          _this.service_list[index][0].highAltitudeSettings.map((arr,index1)=>{
              if(_this.high_value== arr.floorStart+"-"+arr.floorEnd){
              hign_data=arr.price;
