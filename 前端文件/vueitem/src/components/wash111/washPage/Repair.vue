...
<!-- /** 家具维修 维修类型 -->
  <div id="repair">
      <div id="repair_left">
             <ul id="repair_list">
                <li v-for="(item,index) in repair_left_list" @click="repair_listActive(index,item.beans,item.a,item)">
      ...
      <div id="repaire_right">
            <ul id="repaire_right_list">
                ...
                @click="repaire_right_listAction(i,index2)"
...
export default {
  ...
  watch:{//监听到0的数据。修改背景框
    ...
    "choice":function(newchoice,oldchoice){
         console.log(oldchoice,"sbajkdbkasbdkabkdkja")
      console.log(newchoice,"sbajkdbk")
        if(newchoice == 0){
...
//       let url=this.$common.apidomain+'/articleinfo/findlabelbusinessbyflabel?id='+"002";
      ...
      let id=!!this.$route.query.id?this.$route.query.id:this.repair_listActive(0,res.data.result.beans[0].beans)//初始化显示\
      const promise=data=>{
      ...
      if(id){
        promise(res.data.result.beans).then(res=>{
          if(res)return forData(res,6,"repair_listActive");
        }).then(res=>{
          if(res)return forData(res,9,"repaire_right_listAction",false);
        })
      }
    });
  ...
  methods:{
    //点击左侧的li列表
    repair_listActive(index,beans,id,item){
    ...
    repaire_right_listAction(i,index){
      console.log(i, index);
      //跟新对象内的数组数据
