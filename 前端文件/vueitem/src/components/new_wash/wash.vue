...
<!--手机分类模块 start-->
  <!-- 1级分类 -->
    ...
    <div class="container_item third" :class="isPhone&&!!childrenType2&&retractAssortment.isShow?'openBox':'retract'"   v-if="isPhone&&childrenType2!=''&&retractAssortment.isShow">
        <h3 class="assortment_title">{{title[1]}}</h3>
        <ul class="assortment_box" ref="assortment_box">
            <li v-for="(item,index) in childrenType2" :key="index" ref="icon_assortment" @click.stop="selectorTypeFun(index,item.a,item,2)" :class="{'selector_third':i2===index}" :style="item.isClass?'border-radius:2rem':''">
             <!--<i class="icon_assortment" v-if="!item.isClass" :class="!isIconArray[index]?'rotate_icon':'rotate_icon2'"></i>-->
              <span style="font-weight: bold">
                {{item.b}}
              </span>
              <ul class="assortment_children" v-if="childrenType3!=''&&i2===index" :class="{'animated_item':childrenType3!=''&&i2===index}">
                <li v-for="(item,index) in childrenType3" :key="index" :class="{'selector_':i3==index}" @click.stop="selectorTypeFun(index,item.a,item,3)">
                   <b></b>
                   <span class="assortment_children_item">{{item.b}}</span>
                  <!--颜色 start-->
                      <pigment-item v-if="childrenType4!=''&&i3===index" :dataList="childrenType4" :selector="selectorTypeFun" :active="i4"></pigment-item>
                  <!--颜色 start-->
                </li>
              </ul>
            </li>
  ...
  <div class="container_item third "  v-if="!isPhone&&childrenType1!=''&&retractAssortment.isShow" :class="{'openBox':retractAssortment.isShow}">
    ...
    <ul class="assortment_box">
      ...
      <li v-for="(item,index) in childrenType1" :key="index" @click.stop="selectorTypeFun(index,item.a,item,1)" class="househol_d"  :class="{'selector_third animated_item':i1===index}" :style="item.isClass&&!(isKongtiao&&flagIstarges&&i1===index&&childrenType1)?'border-radius:2rem':''">
        ...
        <ul  class="assortment_children" v-if="childrenType2!=''&&i1===index">
          <li v-for="(item,index) in childrenType2" :key="index" :class="{'selector_':i2==index}" @click.stop="selectorTypeFun(index,item.a,item,2)">
            <b></b>
            <span class="assortment_children_item" >{{item.b}}</span>
            <div v-if="(isKongtiao||isAdd)&&i3==index" class="product_price">
              <del v-if="(isKongtiao||isAdd)&&sum&&dataPriceList[0].serviceInfo.isSecondPayment!=='1'">￥{{price2Original}}</del>
              <span>￥{{sum}}</span>
            </div>
            <!--颜色 start-->
            <!--<pigment-item v-if="childrenType4!=''&&i3===index" :dataList="childrenType4" :selector="selectortype4" :active="i4"></pigment-item>-->
            <!--颜色 start-->
          </li>
        </ul>
      </li>
  ...
  export default{
        ...
        methods:{
          ...
          selectortype(index,id,key){
            ...
            this.isIconArray=[];
            if(key.beans){
//              let url = `${common.apidomain}/articleinfo/findlistserviceinfo?labelId=${key.beans[0].a}&areaId=${getSession()[0].id}&token=${getCookie()?getCookie():''}`;
